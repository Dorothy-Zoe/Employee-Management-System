...
// Fetch leave balances dynamically
$sql = "SELECT LeaveType, COALESCE(SUM(UsedLeave), 0) AS used, COALESCE(TotalLeave, 0) AS total
    FROM EmployeeLeaveBalance 
    WHERE EmployeeID = ?
    GROUP BY LeaveType, TotalLeave";
$stmt = $conn->prepare($sql);
if ($stmt) {
    $stmt->bind_param("s", $EmployeeID);
    $stmt->execute();
...
if ($stmt) {
    ...
    $gender = $employeeDetails['Gender'] ?? 'Unknown';
    $stmt->close();
} else {
    die("Error in SQL query: " . $conn->error);
}

// Ensure $defaultLeaveBalances is properly defined
...
foreach ($defaultLeaveBalances as $type => $totalLeave) {
    if (!isset($leaveBalances[$type])) {
    $leaveBalances[$type] = ['used' => 0, 'total' => $totalLeave];
    }
}

// Filter leave balances based on gender
if (strtolower($gender) === 'male') {
    unset($leaveBalances['Maternity Leave']);
} elseif (strtolower($gender) === 'female') {
    unset($leaveBalances['Paternity Leave']);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($consent_for_follow_up == "Yes") {
        // Fetch user's email from EmployeeDetail table
        $sql = "SELECT Email, FName, LName FROM EmployeeDetail WHERE EmployeeID = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $EmployeeID);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    $sql = "INSERT INTO feedback (EmployeeID, feedback_category, feedback_message, screenshot, priority_level, consent_for_follow_up, star_rating)
            VALUES ('$EmployeeID', '$feedback_category', '$feedback_message', '$file_path', '$priority_level', '$consent_for_follow_up', '$star_rating')";
    ...
    <body>
        <div class='email-content'>
            ...
            <p><span class='section-title'>Employee Position:</span> Instructor</p>
...
<!-- Sidebar -->
<?php include_once('../includes/instructorSidebar.php');?>
    <!-- Sidebar -->
...
<!-- End of Notification Modal -->
    ...
    <div class="profile-banner">
        ...
        <div class="banner-content">
            ...
            <div class="banner-details">
                ...
                <p class="banner-role">Instructor</p>
            </div>
        </div>
    ...
    <div class="leave-section-balances">
    <div class="leave-header">
        <h2 class="leave-title">Available Leave Days</h2>
    ...
    <?php foreach ($leaveBalances as $type => $data): ?>
                <div class="leave-type">
                    ...
                    <p class="leave-days-info"><?php echo $data['used']; ?> out of <?php echo $data['total']; ?> day(s)</p>
                    <div class="leave-progress">
                        <div class="leave-progress-bar" style="width: <?php echo ($data['total'] > 0 ? ($data['used'] / $data['total']) * 100 : 0); ?>%"></div>
                    </div>
                </div>
            <?php endforeach; ?>

        <a href="leaveManagement.php" class="apply-leave-btn">
            <span class="icon icon-apply"></span> Apply for leave
        </a>
    </div>
